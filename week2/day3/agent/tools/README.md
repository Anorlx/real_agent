...
- `snip_context`
  - 路径: `agent/tools/context_tools.py`
  - 类别: 上下文
  - 并发安全: 否
...
MCP server 不固定写死在这个 README 里，而是从项目根目录的 `.mcp.json` 动态发现。发现后的工具会注册成：

```text
mcp__<server_name>__<tool_name>
```

当前第一台 MCP server 是 `amap-maps`，通过 stdio 启动 `@amap/amap-maps-mcp-server`。它适合处理高德地图相关任务，例如地址解析、地点搜索、经纬度查询和路线规划。`.mcp.json` 保存本地 key，已经加入 `.gitignore`，不要提交到远端。
...
Terminal 里输入 `/@` 可以手动选择 MCP server，表示“本轮我明确希望使用某个 MCP”。也可以直接输入 `/@amap-maps 查询杭州西湖附近的咖啡店`。
