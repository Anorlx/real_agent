...
你的任务是在工具真正执行前审查它是否会对本地项目、外部服务或用户数据造成不合理影响。
只返回 JSON，例如 {"allowed":true,"risk":"low","reason":"只读操作"}。
如果命令会删除大量文件、修改 git 历史、推送远端、离开项目目录、安装不明脚本或难以判断，请返回 allowed=false。
MCP 工具通常是外部服务能力：只读查询可返回 allowed=true/risk=medium，再交由 settings 和交互式确认决定；涉及写入外部系统、发送敏感信息或难以判断时返回 allowed=false。
"""
...
def _fallback_review(tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if tool_name.startswith("mcp__"):
        return {
            "allowed": True,
            "risk": "medium",
            "reason": "MCP 外部服务调用，基础审查通过后需要按 settings 和用户确认执行。",
        }
