...
def _keyword_fallback(user_input: str, available_tools: dict[str, dict[str, Any]]) -> list[str]:
    ...
    if "/@" in text:
        for name, info in available_tools.items():
            server = str(info.get("mcp_server") or "").lower()
            if info.get("category") == "MCP" and server and f"/@{server}" in text:
                selected.append(name)
        if selected:
            return [name for name in dict.fromkeys(selected) if name in available_names]
        selected.extend(name for name, info in available_tools.items() if info.get("category") == "MCP")
    ...
    if any(word in text for word in ["高德", "地图", "amap", "地址", "路线", "地理编码", "经纬度", "导航"]):
