...
async def _preprocess_node(state: AgentGraphState) -> dict[str, Any]:
    ...
    if _message_token_estimate(state["messages"]) > state["blocking_token_limit"]:
        logger.warning("agent blocking token limit reached")
        return _terminal_update(state, "blocking_limit", TERMINATION_MESSAGES["blocking_limit"])

    next_state: AgentGraphState = dict(state)
    next_state["turn"] = state.get("turn", 0) + 1
    next_state["phase"] = "预处理"
    _emit_state(next_state, "预处理")
...
async def _result_backfill_node(state: AgentGraphState) -> dict[str, Any]:
    ...
    main_agent_saved_memory = any(result.get("name") == "save_memory" for result in tool_results)
    next_state: AgentGraphState = dict(state)
